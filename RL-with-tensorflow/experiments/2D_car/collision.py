...
def intersection():
    ...
    if np.cross(r, s) == 0 and np.cross((q-p), r) == 0:    # collinear cross(r,s)=r*s*sin(theta) then theta==0,pi then two line must be collinear                           
        # t0 = (q − p) · r / (r · r)                                   dot(r,s)=r*s*cos(theta) for 0 cos(theta)>0 and for pi cos(theta) <0
