...
def temporalDifference(states, alpha=0.1, batch=False):
    ...
    while True:
        ...
        if not batch:
            ...
            # When next state value is higher than current one then its update is always +ve
            # states[5] = state[5] + 0.1*(0 + states[6] - states[5]) --> staes[5] = 0.5 + 0.1*(0+1-0.5) = 0.55
            # When next state value is lower than current one then its update is always -ve
            # states[1] = state[1] + 0.1*(0 + states[0] - states[1]) --> staes[5] = 0.5 + 0.1*(0+0-0.5) = 0.45
            #print(states)
